...
pub fn build_plan(
    ...
    for (ci, message) in messages.iter().enumerate() {
        ...
        for (fi, file) in files.iter().enumerate() {
            ...
            let all_included = files[fi].hunks.iter().enumerate().all(|(hi, h)| {
                h.lines
                    .iter()
                    .enumerate()
                    .all(|(li, l)| !l.is_change() || assign[fi][hi][li].is_some_and(|a| a <= ci))
            });
            ...
            let content = apply_file(base, file, &assign[fi], |a| a <= ci);
            changes.push(FileChange {
                path: file.path.clone(),
                mode: file.mode.clone(),
...
",
        ...
        let plan = build_plan(&files, &assign, &bases, &msgs);
        assert_eq!(plan.len(), 2);
        assert_eq!(
            plan[0].files[0].content.as_deref(),
            Some(b"now\n".as_slice())
        );
        assert!(plan[1].files[0].content.is_none());
    }
    ...
    fn plan_skips_empty_commits() {
        ...
        let msgs = vec!["empty".to_string(), "used".to_string()];
        let plan = build_plan(&files, &assign, &bases, &msgs);
        assert_eq!(plan.len(), 1);
        assert_eq!(plan[0].message, "used");
    }
